...
* `GET /api/tasks`: List tasks with optional query filters:
  ...
  * `due_before` (string, optional): Filter tasks due on or before specified date string (e.g. `YYYY-MM-DD`; only tasks with a non-null `due_date <= due_before` are returned; omit to return all tasks).
...
* `PATCH /api/tasks/bulk/status`: Bulk update the status of multiple tasks at once.
  * Body: `BulkStatusUpdate` (`task_ids`: array of integer IDs, `status`: target status string e.g. "Completed").
  * Returns: `BulkStatusResponse` (`updated_count`, `task_ids`, `message`).
